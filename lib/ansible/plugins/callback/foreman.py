...
def get_time():
    """
    Return the time for measuring duration. Prefers monotonic time but falls back to the regular
    time on older Python versions.
    """
    try:
    ...
    def set_options(self, task_keys=None, var_options=None, direct=None):
        ...
        if HAS_REQUESTS:
            if LooseVersion(requests.__version__) < LooseVersion('2.14.0'):
                self._disable_plugin(u'The `requests` python module must be >= 2.14.0.')
        else:
